...
struct LegacyBiasComputer;

impl llguidance::earley::BiasComputer for LegacyBiasComputer {
    fn compute_bias(
        &self,
        _recognizer: &mut llguidance::earley::ParserRecognizer<'_>,
        _start: &[u8],
    ) -> llguidance::toktrie::SimpleVob {
        unreachable!("compile-time compatibility test")
    }
...
#[test]
fn test_bias_computer_greedy_method_is_backward_compatible() {
    fn assert_impl<T: llguidance::earley::BiasComputer>() {}
    assert_impl::<LegacyBiasComputer>();
}

#[test]
fn test_low_level_recognizer_hook_covers_both_backends() {
    let mut ordinary = make_parser(r#"start: "a""#, true).unwrap();
    ordinary.parser.with_any_recognizer(|recognizer| {
        recognizer.trie_started("ordinary_hook");
        assert!(recognizer.try_push_byte(b'a'));
        recognizer.pop_bytes(1);
        recognizer.trie_finished();
    });

    let mut greedy = make_parser(
        r#"
            %llguidance {"no_forcing": true, "greedy_lexeme_fallback": true}
            start: T B
            T: "a" | "abc"
    ...
    .unwrap();
    greedy.parser.with_any_recognizer(|recognizer| {
