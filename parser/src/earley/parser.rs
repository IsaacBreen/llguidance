...
use super::{
    ...
    lexer::{GreedyLexerResult, LexerResult, PreLexeme},
...
struct GreedyParserState {
    ...
    promoted_flush: bool,
    trie_promoted_flush: bool,
    trie_tags_len: usize,
    trie_undos_len: usize,
}
...
impl GreedyParserState {
    fn new(inner: ParserState) -> Self {
        ...
        Self {
            ...
            trie_promoted_flush: false,
            trie_tags_len: usize::MAX,
            trie_undos_len: usize::MAX,
        }
    }
    ...
    fn trie_started_inner(&mut self, lbl: &str) {
        ...
        self.trie_promoted_flush = self.promoted_flush;
        self.inner.trie_started_inner(lbl);
        self.trie_tags_len = self.tags.len();
        self.trie_undos_len = self.undos.len();
    }

    fn trie_finished_inner(&mut self) {
        self.restore_undos_to(self.trie_undos_len);
        self.tags.truncate(self.trie_tags_len);
        ...
        self.promoted_flush = self.trie_promoted_flush;
        self.assert_sidecar();
    }
    ...
    fn rollback(&mut self, n_bytes: usize) -> Result<()> {
    ...
    fn save_state(&self) -> SavedGreedyParserState {
        SavedGreedyParserState {
            lexer_stack_length: self.inner.lexer_stack.len(),
            tags_length: self.tags.len(),
            undos_length: self.undos.len(),
    ...
    fn restore_state(&mut self, state: SavedGreedyParserState) {
        ...
        self.restore_undos_to(state.undos_length);
        self.tags.truncate(state.tags_length);
    ...
    fn validate_tokens(&mut self, tokens: &[TokenId]) -> usize {
        self.inner.assert_definitive();
    ...
    fn increment_token_idx_recursive(&mut self) {
    ...
    fn resize_token_mapping_recursive(&mut self, new_len: usize, value: u32) {
    ...
    fn truncate_token_mapping_recursive(&mut self, new_len: usize) {
        self.inner.byte_to_token_idx.truncate(new_len);
        if let Some(shadow) = self.shadow.as_mut() {
    ...
    fn apply_token(&mut self, tok_bytes: &[u8], tok_id: TokenId) -> Result<usize> {
        self.inner.assert_definitive();
...
pub struct Parser {
    ...
    // Keep the ordinary parser state in exactly the baseline position and layout.
    // Opt-in greedy fallback uses a separate state initialized from this small
    // initial snapshot, so ordinary grammars do not pay for the alternate backend.
    state: ParserState,
    greedy: Option<Box<GreedyParserState>>,
}
...
impl Parser {
    pub fn new(
        ...
        let greedy = state
            ...
            .then(|| Box::new(GreedyParserState::new(state.clone())));
    ...
    #[inline(always)]
    fn uses_greedy(&self) -> bool {
        self.state.grammar.lexer_spec().greedy_lexeme_fallback
    }

    #[inline(always)]
    fn inner(&self) -> &ParserState {
        if self.uses_greedy() {
            &self.greedy.as_deref().unwrap().inner
        } else {
            &self.state
        }
    }
    ...
    #[inline(always)]
    fn inner_mut(&mut self) -> &mut ParserState {
        if self.uses_greedy() {
            &mut self.greedy.as_deref_mut().unwrap().inner
        } else {
            &mut self.state
        }
    }
    ...
    pub fn compute_bias(&mut self, computer: &dyn BiasComputer, start: &[u8]) -> SimpleVob {
        if !self.uses_greedy() {
            ...
            self.with_shared_greedy(|state| state.compute_bias(computer, start))
        }
    }

    pub fn captures(&self) -> &[(String, Vec<u8>)] {
        &self.inner().captures.capture_list
    }

    pub fn get_capture(&self, name: &str) -> Option<&[u8]> {
        self.inner().captures.capture_map.get(name).map(|v| &v[..])
    }
    ...
    pub fn hidden_start(&self) -> usize {
        ...
        self.inner().hidden_start(shared.lexer_mut())
    }
    ...
    pub fn get_error(&self) -> Option<ParserError> {
        ...
        if let Some(e) = &self.inner().parser_error {
    ...
    pub fn chop_tokens(&mut self, trie: &TokTrie, tokens: &[TokenId]) -> (usize, usize) {
        if !self.uses_greedy() {
            self.with_shared(|state| {
                ...
                trie.chop_tokens(&mut recognizer, tokens)
            })
        } else {
    ...
    pub fn force_bytes(&mut self) -> &[u8] {
        if !self.inner().needs_force_bytes() {
            ...
            if !self.uses_greedy() {
                ...
                self.with_shared_greedy(|state| state.force_bytes());
            }
            let r = self.currently_forced_bytes();
            if r.len() > prev_len {
                self.inner().perf_counters.force_bytes.record(t0.elapsed());
            } else {
                self.inner()
                    .perf_counters
                    .force_bytes_empty
    ...
    pub fn scan_eos(&mut self) -> bool {
        if !self.uses_greedy() {
            ...
            self.with_shared_greedy(|state| state.scan_eos())
        }
    }
    ...
    pub fn grammar_warnings(&mut self) -> Vec<String> {
        if !self.uses_greedy() {
            ...
            self.with_shared_greedy(|state| state.inner.lexer_spec().render_warnings())
        }
    }

    pub(crate) fn apply_forced(&mut self, byte_idx: usize) {
        if let Some(state) = self.greedy.as_deref_mut() {
            state.resize_token_mapping_recursive(byte_idx, 0);
        } else {
    ...
    pub(crate) fn additional_backtrack(&mut self, n_bytes: usize) {
        // We can sometimes be asked to backtrack more than we have when the
        // prompt was token-healed; see guidance-ai/guidance#1131.
        let new_len = self.inner().byte_to_token_idx.len().saturating_sub(n_bytes);
        if let Some(state) = self.greedy.as_deref_mut() {
            state.truncate_token_mapping_recursive(new_len);
        } else {
    ...
    pub fn apply_token(&mut self, tok_bytes: &[u8], tok_id: TokenId) -> Result<usize> {
        if !self.uses_greedy() {
            let result = self.with_shared(|state| state.apply_token(tok_bytes, tok_id));
            self.state.token_idx += 1;
            result
        } else {
            let result = self.with_shared_greedy(|state| state.apply_token(tok_bytes, tok_id));
            self.greedy
                .as_deref_mut()
                .unwrap()
                .increment_token_idx_recursive();
            result
        }
    }

    // Keep this body identical to the baseline hot path.
    #[inline(always)]
    fn with_shared<T>(&mut self, f: impl FnOnce(&mut ParserState) -> T) -> T {
        debug_assert!(!self.uses_greedy());
        let mut shared = self.shared.lock().unwrap();
    ...
    #[cold]
    #[inline(never)]
    fn with_shared_greedy<T>(&mut self, f: impl FnOnce(&mut GreedyParserState) -> T) -> T {
        let mut shared = self.shared.lock().unwrap();
        let state = self.greedy.as_deref_mut().unwrap();
        state.inner.shared_box = std::mem::take(&mut *shared);
        let result = f(state);
        *shared = std::mem::take(&mut state.inner.shared_box);
        assert!(shared.lexer_opt.is_some());
        result
    }

    pub fn rollback(&mut self, n_bytes: usize) -> Result<()> {
        self.inner().lexer_spec().check_rollback()?;
        if !self.uses_greedy() {
            self.with_shared(|state| state.rollback(n_bytes))
        } else {
            self.with_shared_greedy(|state| state.rollback(n_bytes))
        }
    }
    ...
    pub fn validate_tokens(&mut self, tokens: &[TokenId]) -> usize {
        if !self.uses_greedy() {
            self.with_shared(|state| {
                let result = state.validate_tokens(tokens);
                debug!(
                    "validate_tokens: {} -> {}/{}",
            ...
            self.with_shared_greedy(|state| {
                let result = state.validate_tokens(tokens);
                debug!("validate_tokens: {}/{}", result, tokens.len());
                result
            })
        }
    }
            ...
            if !self.uses_greedy() {
                ...
                self.with_shared_greedy(|state| {
                    ...
                    debug!(
                        ...
                        label,
                        inner.token_idx,
                        inner.byte_to_token_idx.len(),
                        inner.bytes.len()
                    );
                    for infos in inner.row_infos.iter() {
                        debug!("  {}", infos.dbg(inner.lexer()));
                    }
                })
            }
        }
    ...
    pub fn is_accepting(&mut self) -> bool {
        if !self.uses_greedy() {
            ...
            self.with_shared_greedy(|state| state.is_accepting())
        }
    }

    pub fn currently_forced_bytes(&self) -> &[u8] {
        let state = self.inner();
        &state.bytes[state.byte_to_token_idx.len()..]
    }

    pub fn has_pending_lexeme_bytes(&self) -> bool {
        self.inner().has_pending_lexeme_bytes()
    }
    ...
    pub fn can_advance(&self) -> bool {
        self.inner().can_advance()
    }

    pub fn temperature(&self) -> Option<f32> {
        if let Some(state) = self.greedy.as_deref() {
            state.temperature()
        } else {
    ...
    pub fn test_trigger_lexer_error(&mut self) -> Result<()> {
        if !self.uses_greedy() {
            self.with_shared(|_state| panic!("synthetic error"))
        } else {
            self.with_shared_greedy(|_state| panic!("synthetic error"))
        }
    }

    pub fn invalidate_bias_cache(&mut self) {
        self.inner_mut().bias_cache = None;
    }
}
